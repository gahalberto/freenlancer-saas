...
const UPLOAD_DIR = path.resolve(process.cwd(), 'public/profileavatar')
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Atualizando o avatar_url do usuário no banco de dados com o novo caminho
    const updateUserAvatar = await db.user.update({
      where: {
        id: session.user.id,
      },
      data: {
        avatar_url: `/profileavatar/${fileName}`,
      },
    })

    return res.status(200).json({ 
      success: true, 
      message: 'Upload de avatar bem-sucedido!',
      avatar_url: `/profileavatar/${fileName}`
    })
  } catch (error) {
